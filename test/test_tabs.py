...
def test_tab_visibility(qtbot,main_window,local):
    ...
    def test_tab(name):
        tab_name = name
        tab_action_item = next((a for a in window.tabs_menu_item.actions() if a.text() == tab_name), None)
    ...
    window.show()
    for tab_name in main_window.tabs_toolbox:
        test_tab(tab_name)
    if local:
        qtbot.wait(1000)
...
def test_toolbox_visibility(qtbot,main_window,local):
    ...
    def test_toolbox(name):
        tab_name = name
        tab_action_item = next((a for a in window.toolboxes_menu_item.actions() if a.text() == tab_name), None)
        assert tab_action_item is not None, f"Tab action for '{tab_name}' not found"
