...
class Cloader:
    ...
    def coldBoot(self):
        """Try to get a connection with the bootloader by calling updateInfo
    ...
    def updateInfo(self):
        ...
        if (pk and pk.getHeader() == 0xFF and
    ...
    def flash(self, bufferPage, flashPage, nPage):
        """Initate flashing of data in the buffers to flashPage and nPage
        number of buffers."""
        ...
        pk = CRTPPacket()
        pk.setHeader(0xFF, 0xFF)
        pk.data = struct.pack("<BBHHH", 0xFF, 0x18, bufferPage,
                              flashPage, nPage)
        self.link.sendPacket(pk)

        pk = self.link.receivePacket(1)
        while (not pk or pk.getHeader() != 0xFF or
