...
class InputConfigDialogue(QtGui.QWidget, inputconfig_widget_class):
    ...
    def __init__(self, joystickReader, *args):
        ...
        self.detectThrust.clicked.connect(lambda : self.doAxisDetect("thrust", "Thrust axis",
                                                   "Press the thrust axis to max thrust (also used to adjust target altitude in hover mode)"))
        ...
        self.detectHover.clicked.connect(lambda : self.doButtonDetect("hover", "Hover",
                                                    "Press the button for hover mode activation (releasing returns to manual mode)"))        
    ...
    def _reset_mapping(self):
        self.buttonmapping = {
            ...
            "hover": {"id":-1, "indicator": self.hover},
            }
